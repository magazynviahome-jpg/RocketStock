...
if run_scan:
    ...
    if tickers_df is None or tickers_df.empty:
        ...
        for i, t in enumerate(tickers_list, start=1):
            ...
            if f_hhhl_on and not (hh3_ok and hl3_ok):
                progress.progress(i/len(tickers_list)); continue
        ...
        st.success(f"Skan zakończony. Wyników po twardych filtrach: **{len(raw_results)}**.")

# =========================
# RANKING V2 – MIARODAJNY SCORE 0–100
# =========================
def _clip01(x):
    try:
...
def rank_score_row_v2(row, rsi_min: int, rsi_max: int) -> float:
    ...
    try:
        if slope is not None and atrpct and atrpct > 0:
            rel = float(slope) / (float(atrpct)/100.0)
